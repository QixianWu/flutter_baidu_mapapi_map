...
class BMFMapStatusDispatcher {
  ...
  /// 更新地图属性
  Future<bool> updateMapOptions(MethodChannel _mapChannel, BMFMapOptions change) async {
  ...
  Future<Uint8List?> takeSnapshot(MethodChannel _mapChannel) async {
    ...
    return result != null ? Uint8List.fromList(result.map((i) => i as int).toList()) : null;
  }
  ...
  /// 室内图标注是否显示
  Future<bool> showBaseIndoorMapPoi(MethodChannel _mapChannel, bool showPoi) async {
    ArgumentError.checkNotNull(_mapChannel, "_mapChannel");
  ...
  Future<bool> setCustomMapStyleWithOption(
    ...
    try {
      ...
      result = map != null;
      if (map != null) {
        if (preload != null && map['preloadPath'] != null) { // ignore: unnecessary_null_comparison
          preload(map['preloadPath']);
        }
        if (success != null && map['successPath'] != null) { // ignore: unnecessary_null_comparison
          success(map['successPath']);
        }
        if (error != null) { // ignore: unnecessary_null_comparison
